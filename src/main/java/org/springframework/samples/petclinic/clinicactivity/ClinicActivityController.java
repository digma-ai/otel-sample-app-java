...
@RequestMapping("/api/clinic-activity")public class ClinicActivityController implements InitializingBean {
    ...
    @Operation(summary = "Get paginated clinic activity logs", description = "Retrieves clinic activity logs with pagination support")
    public Page<Map<String, Object>> getLogs(
        ...
        for (int i = 0; i < repetitions; i++) {
            lastResults = jdbcTemplate.queryForList(sql + " LIMIT " + pageable.getPageSize() + " OFFSET " + pageable.getOffset(), numericValueToTest);
        }
        
        long total = jdbcTemplate.queryForObject(countSql, Long.class, numericValueToTest);
        return new PageImpl<>(lastResults, pageable, total);
