...
@RequestMapping("/api/clinic-activity")public class ClinicActivityController implements InitializingBean {
    ...
    public Page<Map<String, Object>> getLogs(
        ...
        for (int i = 0; i < repetitions; i++) {
            lastResults = jdbcTemplate.queryForList(
                sql, 
                new Object[]{numericValueToTest, pageable.getPageSize(), pageable.getOffset()},
                new int[]{Types.INTEGER, Types.INTEGER, Types.INTEGER}
            );
        }
        
        int total = jdbcTemplate.queryForObject(countSql, Integer.class, numericValueToTest);
