...
@RequestMapping("/api/clinic-activity")public class ClinicActivityController implements InitializingBean {
    ...
    }@Cacheable(value = "queryLogs", key = "#numericValue + '-' + #page + '-' + #size")
...
public ResponseEntity<Map<String, Object>> getLogs(
    ...
    String sql = "SELECT id, activity_type, numeric_value, event_timestamp " +
                "FROM clinic_activity_logs " +
                "WHERE numeric_value = ? " +
                "LIMIT ? OFFSET ?";
    
    int offset = page * size;
    List<Map<String, Object>> results = jdbcTemplate.queryForList(sql, numericValue, size, offset);
    
    // Get total count for pagination
    String countSql = "SELECT COUNT(*) FROM clinic_activity_logs WHERE numeric_value = ?";
    int totalItems = jdbcTemplate.queryForObject(countSql, Integer.class, numericValue);
